...
Roles generated by this module can only be assumed within the github actions associated with the given git `origin` attribute.

When assigning policies to these roles ensure you are leveraging least priviledge. Consider it an exercise in accurately defining the activities of the stage.

The ECR management role is given write access to the ECR repositories provided and is only available within the ECR account. 

Cross account access policies applied to the given ECR repositories are for read only access.
...
The topology module...
1. Configures ECR repositories for cross account access.
...
module "topology" {
    source = "github.com/bkeane/stage/topology?ref=<version>"
...
module "deploy" {
    source = "github.com/bkeane/stage/stage?ref=<version>"
...
The composite actions output by `module.topology` provides a simple mechanism to setup these stages in a Github Actions workflow.
...
As mentioned previously, the topology module injects a single stage under the same account the topology module is applied. The name can be modified with the `ecr_stage_name` attribute and by default the stage is named `build`.
...
Unlike other stages, this stage is only made available in the ECR account.
